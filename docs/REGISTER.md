...
| EN-036 | Default memory landscape declarations | EN-026 | built | | scripts/validate_mvp_operator_console.py | hyperscalers + leading frameworks ship declared by default (Vertex Memory Bank, AgentCore Memory, mem0, Zep, Letta, Honcho): visible, disabled, adapterless-honest until configured; adapter implementations follow the deep-research ranking |
| EN-037 | Vertex Memory Bank adapter | IDEA-023 | built | +00+ | scripts/validate_mvp_vertex_adapter.py | U: first remote extractive backend, top-ranked (immutable revisions); M: deterministic normalization + hash-at-observation verify (stale = service revised the fact); peer memory type routed; ships installed-but-disabled (remote, default_enabled False, reset preserves off); live GCP client is the next inch |
| EN-038 | Vertex live client boundary | EN-037 | built | +00+ | docs/VERTEX_MEMORY_BANK.md | U: first live remote backend — real create/get/retrieve against Memory Bank; M: zero-dep gcloud-token + REST boundary, honest degradation without engine config; proven live 2026-07-20: write verified, server-side revision caught stale, fanout lane live with attribution |
...
| IDEA-023 | Managed extractive memory backend | EN-013 | idea | | research/managed-memory-landscape-2026-07-19.md | verified ranking: 1. Vertex Memory Bank (immutable per-mutation revisions + rollback = best provenance), 2. AWS AgentCore Memory (GA Oct 2025, per-record ids, no revision history); mem0/Zep/Letta/Honcho web claims failed verification — evaluate hands-on; graduated to EN-037 |
| IDEA-024 | OpenReflect memory company thesis | — | idea | | docs/strategy/openreflect-memory-company-thesis.md | own AI memory across infrastructure, meaning, experience, learning, and public understanding without making memory a vague umbrella |
